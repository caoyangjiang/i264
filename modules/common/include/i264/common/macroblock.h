...
class Macroblock {
 public:
  ...
  void StoreLumaResidual(Array2D<int, 4, 4> residual, int zscan_index);
  void StoreChromaResidual(Array2D<int, 8, 8> residual);
  void StoreChromaResidual(Array2D<int, 2, 2> residual, int zscan_index);

  // go into recon buffer. recon = residual + deblocking
  void StoreLumaRecon(Array2D<int, 16, 16> residual);
  void StoreLumaRecon(Array2D<int, 4, 4> residual, int zscan_index);
  void StoreChromaRecon(Array2D<int, 8, 8> residual);
  void StoreChromaRecon(Array2D<int, 2, 2> residual, int zscan_index);
